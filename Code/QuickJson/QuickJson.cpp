...
QuickJson* QuickJson::addNode() {
    QuickJson* node = new QuickJson();
...
QuickJson* QuickJson::ParseString( std::string str ) {
    ...
    QuickJson* node;
    std::vector< QuickJson* > stack;

    if( size > 0 ) {
        node = new QuickJson();
        stack.push_back(node);
        QuickJsonParser* parser = new QuickJsonParser();

        int pos = 0;

        while( ( pos < size ) && parser->Good() ) {
            while( parser->Reading() ) {
                parser->Append(str[pos]);
                pos++;
            }
            if( parser->Jump() ) {
                node = stack.pop_back();
                ...
                if( parser->Bottom() ) {
                    stack.back()->addField(parser->Key(), parser->Value());
                } else {
                    node = stack.back()->addNode(parser->Key());
                    stack.push_back(node);
                }
            }
        }
